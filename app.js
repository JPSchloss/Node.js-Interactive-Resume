...
import skillsText from 'raw-loader!./skillsets.txt';
...
const speed = isDev ? 10 : 16;
let style, styleEl, workEl, skillsEl, skipAnimationEl, pauseEl;
...
async function startAnimation() {
  try {
    // Intro To Site Using styles0.css
    await writeTo(styleEl, styleText[0], 0, speed, true, 1);

    // Bringing In Resume as work.txt
    await writeTo(workEl, workText, 0, speed, false, 32);

    // Updating The Markdown Using styles1.css
    await writeTo(styleEl, styleText[1], 0, speed, true, 1);
    createWorkBox();
    await Promise.delay(1000);

    // Introducing the Skills Text with styles2.css
    await writeTo(styleEl, styleText[2], 0, speed, true, 1);

    // Writing the skills section using skillsets.txt
    await writeTo(skillsEl, skillsText, 0, speed, false, 32);
    createSkillsBox();
    await Promise.delay(1000);
...
async function surprisinglyShortAttentionSpan() {
  ...
  while(Date.now() - 1000 > start) {
    ...
    styleEl.scrollTop = skillsEl.scrollTop = Infinity;
...
function getEls() {
  ...
  skillsEl = document.getElementById('skills-text');
...
function createSkillsBox() {
  if (skillsEl.classList.contains('flipped')) return;
  skillsEl.innerHTML = '<div class="text">' + replaceURLs(skillsText) + '</div>' +
                     '<div class="md">' + replaceURLs(md(skillsText)) + '<div>';

  skillsEl.classList.add('flipped');
  skillsEl.scrollTop = 9999;

  // modify this dont like the flippy floppy
  // flippy floppy
  let flipping = 0;
  require('mouse-wheel')(skillsEl, async function(dx, dy) {
    if (flipping) return;
    let flipped = skillsEl.classList.contains('flipped');
    let half = (skillsEl.scrollHeight - skillsEl.clientHeight) / 3;
    let pastHalf = flipped ? skillsEl.scrollTop < half : skillsEl.scrollTop > half;

    // If we're past half, flip the el.
    if (pastHalf) {
      skillsEl.classList.toggle('flipped');
      flipping = true;
      await Promise.delay(500);
      skillsEl.scrollTop = flipped ? 0 : 9999;
      flipping = false;
    }

    // Scroll. If we've flipped, flip the scroll direction.
    skillsEl.scrollTop += (dy * (flipped ? -1 : 1));
  }, true);
}
